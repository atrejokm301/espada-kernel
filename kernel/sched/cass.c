 ...
 */
/*
 * PORT NOTE (grizzly / spacecraft, 6.1.145 -> 6.12.69, 2026-08-25)
 *
 * Taken verbatim from Sultan's 16.0.0-sultan branch at 201c5c7c99f8 (which is
 * the tip for this file and already contains every follow-up fix), with only
 * the two changes that 6.12 requires:
 *
 *   1. cfs_rq->avg.util_est.enqueued -> cfs_rq->avg.util_est
 *      v6.8 collapsed struct util_est into a plain unsigned int on sched_avg.
 *   2. thermal_load_avg(rq) -> hw_load_avg(rq)
 *      the thermal_* PELT accessors were renamed hw_* upstream; the 6.12
 *      version is defined in both CONFIG_SCHED_HW_PRESSURE branches of pelt.h
 *      and returns the same rq->avg_hw.load_avg. (=y in our build.)
 *
 * Deliberately NOT changed, to keep the diff against upstream minimal:
 *   cass_select_task_rq() tests wake_flags against SD_BALANCE_EXEC/_FORK rather
 *   than WF_EXEC/WF_FORK. These are numerically identical by design -- see the
 *   ordering in include/linux/sched/sd_flags.h (SD_BALANCE_EXEC = 0x02 = WF_EXEC,
 *   SD_BALANCE_FORK = 0x04 = WF_FORK) and the "maps to" comments in sched.h.
 *
 * Requires the prerequisite commit 48a3fd5e414d ("arch_topology: Introduce
 * minimum frequency scale"), which supplies arch_scale_min_freq_capacity();
 * without it this file does not build.
 */
